...
async def profile_shop_buffs(callback: types.CallbackQuery):
    ...
    buffs_config = [
        ...
        (
            "intercept",
            t.BUFF_INTERCEPT_NAME,
            t.BUFF_INTERCEPT_DESC,
            inv.get("intercept", 0),
            t.BUFF_INTERCEPT_PRICE,
            125,
        ),
        (
            "detector",
            t.BUFF_DETECTOR_NAME,
            t.BUFF_DETECTOR_DESC,
            inv.get("detector", 0),
            t.BUFF_DETECTOR_PRICE,
            75,
        ),
        (
            "reveal",
        ...
        (
            "become_captain",
            t.BUFF_BECOME_CAPTAIN_SHORT,
            t.BUFF_BECOME_CAPTAIN_DESC,
            inv.get("become_captain", 0),
            t.BUFF_BECOME_CAPTAIN_PRICE,
            t.BUFF_BECOME_CAPTAIN_PRICE_COINS,
        ),
        (
            "avoid_captain",
            t.BUFF_AVOID_CAPTAIN_SHORT,
            t.BUFF_AVOID_CAPTAIN_DESC,
            inv.get("avoid_captain", 0),
...
async def buy_inv_buff(callback: types.CallbackQuery):
    ...
    if currency == "dia":
        ...
        balance = await db_service.get_user_diamonds(callback.from_user.id)
        logger.info(
            f"BUY_DIA: user={callback.from_user.id}, buff={buff_type}, price={price}, balance={balance}"
        )
        if balance < price:
            logger.warning(
                f"BUY_DIA_FAILED: user={callback.from_user.id}, balance={balance} < price={price}"
            )
        ...
        logger.info(
            f"BUY_COIN: user={callback.from_user.id}, buff={buff_type}, price={price}, balance={balance}"
        )
        if balance < price:
            logger.warning(
                f"BUY_COIN_FAILED: user={callback.from_user.id}, balance={balance} < price={price}"
            )
            return await callback.answer(t.BUY_FAIL, show_alert=True)
